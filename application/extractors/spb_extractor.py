...
class SPBExtractor:
    ...
    def extrair_dados_pdf(self, pdf_file: BytesIO) -> dict:
        ...
        try:
            ...
            padrao_cnpj = r'CPF/CNPJ:\s*([\d{2}\.\d{3}\.\d{3}/\d{4}-\d{2}]+)'
            cnpj_match = re.search(padrao_cnpj, texto_pagina1)
            cnpj = cnpj_match.group(1) if cnpj_match else None

            # Extrair Município na segunda página
            padrao_cidade = r'Município:\s*([A-Za-zÀ-ÿ\s-]+)'
            cidade_match = re.search(padrao_cidade, texto_pagina2)
            cidade = cidade_match.group(1).strip() if cidade_match else None

            # Extrair Valor Total na segunda página
            padrao_valor = r'VALOR DO DOCUMENTO\s*([\d.,]+)'
            valor_match = re.search(padrao_valor, texto_pagina2)
        ...
        try:
            ...
            try:
                arquivo_existente = self.sharepoint_auth.baixar_arquivo_sharepoint(
                    nome_arquivo_excel, pasta_consolidado
                )
            ...
            # Envia arquivo consolidado para SharePoint
            sucesso = self.sharepoint_auth.enviar_para_sharepoint(
                arquivo_consolidado, nome_arquivo_excel, pasta_consolidado
            )

            if sucesso:
                print(f"✅ Arquivo {nome_arquivo_excel} salvo com sucesso na pasta CONSOLIDADO")
